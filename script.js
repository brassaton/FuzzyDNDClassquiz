...
function renderResults(){
  ...
  // Keep most recommendations focused on the winning class, but use the player's
  // next-highest class results for slots 2, 5, and 6 so the list has broader coverage.
  // Slot 2 = second-ranked class, slot 5 = third-ranked class, slot 6 = fourth-ranked class.
  const winnerSubs=subs.slice(0,3).map(x=>({...x,cls:winner}));
  ...
  const alternateSubs=alternateClasses.map(c=>{
    ...
    return ranked.length ? {...ranked[0],cls:c.name} : null;
  });
  const matchEntries=[
    winnerSubs[0],
    alternateSubs[0],
