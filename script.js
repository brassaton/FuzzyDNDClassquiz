...
const CLASS_PROFILES = {
  ...
  Bard:{skills:6,support:5,utility:5,control:3,healing:2,mobility:2,range:2,burst:2,defense:1,aoe:2,sustained:2,singleTarget:1,tank:0,chaos:1},
  ...
  Ranger:{skills:6,range:5,singleTarget:4,sustained:4,mobility:4,utility:3,burst:3,control:2,aoe:2,defense:2,support:2,healing:0,tank:1,chaos:0},
  Rogue:{skills:6,singleTarget:5,burst:5,mobility:4,utility:4,sustained:3,range:3,control:2,aoe:1,defense:1,support:1,healing:0,tank:0,chaos:1},
  ...
  Artificer:{skills:6,utility:5,support:4,defense:4,range:3,sustained:3,control:3,aoe:2,singleTarget:2,burst:2,mobility:2,healing:1,tank:3,chaos:1},
...
const FINAL_FANTASY_QUESTIONS = [
  ...
  {category:"Class Fantasy",trait:"skills",phase:"final",weight:10,text:"Do you want to be good at skills or tools?",answers:[
    ["YES",{traits:{skills:7,utility:1},classes:{Bard:4,Rogue:4,Artificer:4,Ranger:4},weight:10}],
    ["NO",{traits:{skills:-1},classes:{Bard:-1,Rogue:-1,Artificer:-1,Ranger:-1},weight:10}]
  ]},
...
function classRanking(player){
  return Object.keys(CLASS_PROFILES).map(cls=>({
    name:cls,
    score:signedCosine(player.traits,CLASS_PROFILE_NORMALIZED[cls]) + ((player.classes[cls]||0)*0.025)
...
function returnFromSubclassDetail(){
  if(detailReturnScreen==="results-screen"){
...
function renderResults(){
  ...
  document.getElementById('subclass-matches').innerHTML=subs.slice(0,6).map((x,i)=>`
    <button class="match clickable-match" type="button" data-detail-class="${escapeHtml(winner)}" data-detail-sub="${escapeHtml(x.name)}">
      <span><b>${i+1}. ${escapeHtml(x.name)}</b>${x.custom?" <span class='mini-custom'>CUSTOM</span>":''}</span>
      <span class="match-arrow">→</span>
    </button>`).join('');

  document.querySelectorAll('.clickable-match').forEach(btn=>{
    btn.addEventListener('click',()=>renderSubclassDetail(btn.dataset.detailClass,btn.dataset.detailSub,"results-screen",subs));
  });
